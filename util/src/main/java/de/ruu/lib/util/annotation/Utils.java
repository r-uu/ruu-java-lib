...
public abstract class Utils
{
	...
	private Utils() { throw new AssertionError("utility class"); }

	public static <A extends Annotation> Optional<A> findAnnotation(
			AnnotatedElement element, @NonNull Class<A> annotationType)
	{
			...
			Set<Annotation> visited)
	{
		...
		if (directMetaAnnotation.isPresent()) { return directMetaAnnotation; }
		...
		if (element instanceof Class)
		{
			...
			for (Class<?> ifc : clazz.getInterfaces())
			{
				if (ifc != Annotation.class)
				{
					...
					if (annotationOnInterface.isPresent()) { return annotationOnInterface; }
				}
			}
			...
			if (inherited)
			{
				...
				if (superclass != null && superclass != Object.class)
				{
					...
					if (annotationOnSuperclass.isPresent()) { return annotationOnSuperclass; }
				}
			}
		}
			...
			Set<Annotation> visited)
	{
		for (Annotation candidateAnnotation : candidates)
		{
			...
			if (!isInJavaLangAnnotationPackage(candidateAnnotationType) && visited.add(candidateAnnotation))
			{
				...
				if (metaAnnotation.isPresent()) { return metaAnnotation; }
			}
		}
